...
class CountTest extends TestCase
{
    ...
    {
        // Covers R13.
        $this->assertSame(0, new Count()->forPage(new Collection()));
    }

    public function test_an_empty_query_has_no_rows_to_aggregate(): void
    {
        // Covers R13, AE7.
        $this->assertSame(0, new Count()->forQuery(TestModel::query(), 'amount'));
    }

    public function test_it_aggregates_the_values_on_the_page(): void
    {
        $this->assertSame(3, new Count()->forPage(new Collection([10, 20, 30])));
    }
    ...
    public function test_it_aggregates_the_whole_query(): void
    {
        ...
        $this->assertSame(3, new Count()->forQuery(TestModel::query(), 'amount'));
    }

    public function test_it_ignores_null_values_on_the_page(): void
    {
        $this->assertSame(
            new Count()->forPage(new Collection([10, 20, 30])),
            new Count()->forPage(new Collection([10, null, 20, null, 30])),
        );
    }

    public function test_it_is_an_aggregate(): void
    {
        $this->assertInstanceOf(Aggregate::class, new Count());
    }

    public function test_it_declares_whether_it_carries_the_column_unit(): void
    {
        $this->assertFalse(new Count()->carriesColumnUnit());
    }
