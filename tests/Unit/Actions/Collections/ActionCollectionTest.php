...
class ActionCollectionTest extends TestCase
{
    ...
    public function test_nest_adds_the_given_collection(): void
    {
        $collection = new ActionCollection([new Action()]);
        $nested     = new ActionCollection([new Action()]);

        $collection->nest($nested);

        $this->assertCount(2, $collection);
        $this->assertSame($nested, $collection->last());
    }

    public function test_nest_returns_the_collection_for_chaining(): void
    {
        $collection = new ActionCollection();

        $this->assertSame($collection, $collection->nest(new ActionCollection()));
    }

    public function test_flatten_returns_a_flat_collection_of_actions(): void
    {
        $first  = new Action();
        $second = new Action();
        $third  = new Action();
        ...
        $collection = new ActionCollection([
            $first,
            new ActionCollection([$second, $third]),
        ]);

        $flattened = $collection->flatten();

        $this->assertInstanceOf(ActionCollection::class, $flattened);
        $this->assertCount(3, $flattened);
        $this->assertSame([$first, $second, $third], $flattened->all());
    }

    public function test_flatten_handles_deeply_nested_collections(): void
    {
        $collection = new ActionCollection([
            new Action(),
            new ActionCollection([
        ...
        $this->assertCount(3, $collection->flatten());
    }

    public function test_flatten_leaves_the_original_collection_untouched(): void
    {
        $collection = new ActionCollection([
            new Action(),
            new ActionCollection([new Action(), new Action()]),
    ...
    public function test_flatten_of_an_empty_collection_is_empty(): void
    {
        $this->assertCount(0, new ActionCollection()->flatten());
    }
