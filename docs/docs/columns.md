...
You can make the Table sortable by a given column by setting the `sortable` option to `true`.
...
When a column is sortable, the Table will sort by the name of the column, like `$builder->orderBy($column->name, $request->sortDirection)`.

### Default sort

If you want to set a default sort direction for a column, you can set the `defaultSort` option to either 
`BrickNPC\EloquentTables\Enums\Sort::Asc` or `BrickNPC\EloquentTables\Enums\Sort::Desc`, or a `Closure` that performs 
the sorting.
...
new Column(name: 'name', defaultSort: fn(Request $request, Builder $builder) => $builder->orderBy('lastname', 'asc'));
// Or
new Column(name: 'name')->sortable(default: fn(Request $request, Builder $builder) => $builder->orderBy('lastname', 'asc'));
```
...
By default, the Table will sort the table by the name of the column. But if you want to sort by a different column on 
the Model or use a custom sorting algorithm, you can use the `sortUsing` option.
...
new Column(
    name: 'name', 
    sortUsing: fn(Request $request, Builder $builder, Sort $direction) => $builder->orderBy('lastname', $direction->value), 
);
// Or
new Column(name: 'name')
    ->sortable(
...
You can make the Table searchable by marking at least one column as searchable by setting the `searchable` option to 
`true`.
...
new Column(
    name: 'name', 
    searchable: true, 
    searchUsing: fn(Request $request, Builder $builder, string $search) => $builder->where('lastname', 'LIKE', "%{$search}%"), 
);
// Or
new Column(name: 'name')
    ->searchable(
...
app config: `app.timezone`. It uses the current locale of the app for the locale: `app()->getLocale()`.
...
number of decimals from the app config: `app.decimals`. It uses the current locale of the app for the locale: 
...
default currency from the app config: `app.currency`. It uses the current locale of the app for the locale:
...
You can `styles` option for the Table styles. The `styles` option expects an array of 
